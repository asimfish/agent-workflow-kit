...
- **评审是强制的，不是请求。** 合并需要 CI 全绿，加上一个从未碰过这份改动的
  会话给出的批准。运行时检查在同一个仓库内是权威的（gate 能读到干活者自己的
  会话记录）；在另一个克隆里它只和提交进来的记录一样可信，所以跨机器的评审
  仍然依赖评审者确实是另一个人或另一个会话。
...
305 个回归测试在 Linux 的 CI 上运行，另有一个 Windows job 跑其中涉及 Windows
