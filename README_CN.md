...
- **评审是强制的，不是请求。** 合并需要 CI 全绿，加上一个从未碰过这份改动的
  会话给出的批准。运行时检查在同一个仓库内是权威的：`finish` 还会把干活者的
  运行时按任务的键记录在本地，gate 会读它，所以即使提交进来的文本被改过，
  自己评审自己在这里也会被拒；在另一个克隆里只有提交进来的记录，所以跨机器
  的评审仍然依赖评审者确实是另一个人或另一个会话。
...
307 个回归测试在 Linux 的 CI 上运行，另有一个 Windows job 跑其中涉及 Windows
