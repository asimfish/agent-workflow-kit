...
- **评审是强制的，不是请求。** 合并需要 CI 全绿，加上一个从未碰过这份改动的
  会话给出的批准。在同一个仓库内这个检查是权威的：对任务的每一次认领、记录、
  释放或完成，都会把该会话的运行时按任务的键记在本地，gate 拒绝其中任何一个
  运行时来当评审者，无论提交进来的记录怎么写；在另一个克隆里只有提交进来的
  记录，所以跨机器的评审仍然依赖评审者确实是另一个人或另一个会话。
...
309 个回归测试在 Linux 的 CI 上运行，另有一个 Windows job 跑其中涉及 Windows
